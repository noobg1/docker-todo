...
class Container extends Component {
  ...
  addItem(newTodoDescription) {
    axios.post(`http://localhost:8080/api/write/${this.escapeHtml(newTodoDescription)}`)
      .then((response) => {
        let newTodo = {
          id: response.data.id,
          status: false,
  ...
  updateTodo(todo, updatedDesciption, status) {
    ...
    axios.put(`http://localhost:8080/api/update/${todo.id}`, qs.stringify(toUpdate))
      .then((response) => {
        todo.status = status
  ...
  destroyTodo(todo) {
    axios.delete(`http://localhost:8080/api/delete/${todo.id}`)
      .then((response) => {
        this.setState((oldState) => {
